...
% (6) Value Function Iteration with Multigrid
%
% Felipe Ruiz Mazin
% December 10, 2018
...
sizeCapital           = [100 500 5000]; % number of points in grid for capital
...
valueFunction0        = utilitySteadyState.*ones(sizeCapital(1),sizeShocks);
inputs.valueFunction0 = valueFunction0;

for i = 1:3
    
    % Required matrices and vectors
    valueFunction         = ones(sizeCapital(i),sizeShocks);
    policyFunction        = zeros(sizeCapital(i),sizeShocks);
    indexFunction         = ones(sizeCapital(i),sizeShocks);
    laborFunction         = laborSteadyState.*ones(sizeCapital(i),sizeShocks);
    consumptionFunction   = zeros(sizeCapital(i),sizeShocks);
    inputs.laborFunction  = laborFunction;

    diff                  = zeros(100,1);
    gridCapital           = linspace(capitalMin, capitalMax, sizeCapital(i));
    inputs.gridCapital    = gridCapital;
    inputs.simulParams(1) = sizeCapital(i); 
    ...
    while iter <= maxIter && diff(iter) > tol
        inputs.expectedValue0 = (valueFunction0.^ppsi) * transitionMatrix';
        for indexShocks = 1:sizeShocks
            for indexCapital = 1:sizeCapital(i)
                [policyFunction(indexCapital,indexShocks), vAux] = fminbnd(@(capitalPrime) ...
                    -value_function(capitalPrime,indexCapital,indexShocks),...
                    max(0.8*gridCapital(indexCapital),capitalMin),min(1.2*gridCapital(indexCapital),capitalMax),options);
                valueFunction(indexCapital,indexShocks) = -vAux;
                if (diff(iter) < tol*10) || (i == 3)
    ...
    if i ~= 3
        valueFunction0         = interp1(gridCapital,valueFunction,linspace(capitalMin, capitalMax, sizeCapital(i+1)));
        inputs.valueFunction0  = valueFunction0;
        valueFunction          = valueFunction0;
        policyFunction0        = interp1(gridCapital,policyFunction,linspace(capitalMin, capitalMax, sizeCapital(i+1)));
    end
...
zPath = zeros(1,T-1)+0;
zPath(1) = gridProd(5);
alphaPath = zeros(1,T-1)+0.3;
alphaPath(1) = gridAlpha(3);

figure
subplot(2,2,1)
plot(impulseRespCapitalProdPD(2:end))
hold on
plot(zeros(1,T-1),'k')
xlim([1, T])
title('Capital')
...
subplot(2,2,2)
plot(impulseRespLaborProdPD(2:end))
title('Labor')
%xlabel('time')
ylabel('% deviation from ss')
hold on
...
xlim([1, T])
subplot(2,2,3)
plot(impulseRespConsProdPD(2:end))
title('Consumption')
xlabel('time')
ylabel('% deviation from ss')
hold on
...
plot(zPath)
hold on
plot(zeros(1,T-1)+0,'k')
xlim([1, T])
title('Productivity')
xlabel('time')
ylabel('z (level)')
saveas(gcf,'IRFz_MG.png')

figure
subplot(2,2,1)
plot(impulseRespCapitalSharePD(2:end))
hold on
plot(zeros(1,T-1),'k')
xlim([1, T])
title('Capital')
...
ylabel('% deviation from ss')
subplot(2,2,2)
plot(impulseRespLaborSharePD(2:end))
hold on
plot(zeros(1,T-1),'k')
xlim([1, T])
title('Labor')
...
ylabel('% deviation from ss')
subplot(2,2,3)
plot(impulseRespConsSharePD(2:end))
hold on
plot(zeros(1,T-1),'k')
xlim([1, T])
title('Consumption')
...
plot(alphaPath)
hold on
plot(zeros(1,T-1)+0.3,'k')
xlim([1, T])
title('Capital Share')
xlabel('time')
ylabel('\alpha (level)')
saveas(gcf,'IRFalpha_MG.png')