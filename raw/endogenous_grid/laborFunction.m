...
% Now in order to use fminbnd to specify the lower and upper bound for
% solving labor, I changed the value function
% y = ...
%     (labor(1) + labor(2) - (a_2*labor(2)).^mmu_2*mmu_1.*(a_1*k.^aalphaK.*labor(1).^aalphaL + (1-ddelta)*k-kPrime).^(mmu_1-1).*a_1.*k.^aalphaK*aalphaL*labor(1).^(aalphaL-1)).^2  + ( labor(1) + labor(2) - (a_1*k.^aalphaK*labor(1)^aalphaL + (1-ddelta)*k-kPrime).^(mmu_1)*a_2.^mmu_2*mmu_2*labor(2).^(mmu_2-1)).^2;
